...
class RoadSectionManager(object):
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.road_sections = [StaticRoadSection(
                0, previeous_section=section_neg_one, static_obstacle_pos=[])]
...
class RoadSection(pygame.sprite.Sprite):
    def __init__(self, index: int, surface, previeous_section: 'RoadSection', next_section: 'RoadSection'):
        super().__init__()
        self.image = surface
    ...
    def get_sections_to_draw(self):
        ...
        self.sections_to_draw = [
            self.previous_section.previous_section, self.previous_section, self]
        next_section = self
...
class StaticRoadSection(RoadSection):
    def __init__(self, index: int, previeous_section: RoadSection = None, next_section: RoadSection = None, static_obstacle_pos: int = None):
        image = pygame.Surface(
            (config.WINDOW_WIDTH, config.BLOCK_SIZE), pygame.SRCALPHA)
        if static_obstacle_pos is None:
            static_obstacle_pos = random.sample(range(
                0, config.ROAD_COLUMNS+2*config.UNSTEPABLEE_COLUMNS), random.randint(1, config.ROAD_COLUMNS//4))
        if (index % 2 == 0):
    ...
    def move_possible(self, player: pygame.sprite.Sprite, move):
        ...
        out_put = pygame.sprite.spritecollide(
            player, self.static_obstacles, False)
    ...
    def draw(self, surface, y_offset):
        ...
        for static_obstacle in self.static_obstacles:
            surface.blit(static_obstacle.image,

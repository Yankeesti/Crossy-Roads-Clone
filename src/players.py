...
class PlayerManager(object):
    ...
    def __new__(cls, controllers=[key_handler.HumanController()]):
        if cls._instance is None:
            ...
            cls._instance.players = [Player(
                cls._instance.road_section_manager.road_sections[0], controller, cls._instance) for controller in controllers]
...
class Player(pygame.sprite.Sprite):
    def __init__(self, currentSection: map.RoadSection, controller, manager: PlayerManager):
        ...
        self.rect.bottomleft = (
            (config.ROAD_COLUMNS // 2)*config.BLOCK_SIZE, 0)
    ...
    def init_move_up(self):
        if self.check_move_possible([self.sections[0].next_section], (0, -config.BLOCK_SIZE)):
            ...
            self.moves.put(lambda: (
                ...
                setattr(self, "killing_y_point",
                        self.sections[0].rect.bottomleft[1] + config.MAX_BLOCKS_BACK*config.BLOCK_SIZE),
            ))
